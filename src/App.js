...
function App() {
  ...
  const [copied, setCopied] = useState(false); // State for clipboard animation
  ...
  const copyToClipboard = () => {
    navigator.clipboard.writeText(sweetSpot.toFixed(4)).then(() => {
      setCopied(true);
  ...
  return (
    <div className={`relative flex flex-col items-center justify-center h-screen ${theme.background}`}>
      ...
      <div className="mb-6 flex sm:w-full sm:justify-center">
        <label htmlFor="skillInput" className={`text-lg font-medium ${theme.text}`}>
      ...
      <div className="mb-6 text-lg flex items-center space-x-2">
        <p>
          <span className={`font-medium ${theme.text}`}>Exact Sweet Spot:</span>{" "}
          <span className="font-bold text-red-400">{sweetSpot.toFixed(4)}</span>
        </p>
        <button
          onClick={copyToClipboard}
          className="p-2 hover:bg-gray-400 rounded-lg  text-white shadow-lg hover:animate-pulse active:bg-gray-600 transition-all duration-200"
        >
          {copied ? "Copied ✓" : "📋"}
        </button>
      </div>
